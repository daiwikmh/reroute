...
import { listActiveEndpoints, resolveDomain } from "./dns/browse.js";
import { handlePay } from "./x402/proxy.js";
import { syncOnce } from "./dns/sync.js";
import { OFFRAMP_COUNTRIES } from "./offramp/countries.js";
import { configureOffRamp, getOffRampSummary, withdraw } from "./offramp/index.js";
...
// Live "look up any domain" search — a single DNS TXT lookup, not limited to
// domains already known to our own KV state.
app.get("/resolve/:domain", async (c) => {
  const domain = c.req.param("domain").trim().toLowerCase();
  const entry = await resolveDomain(domain);
  if (!entry) return c.json({ error: "No AID record found — not registered with Reroute, or DNS hasn't verified yet." }, 404);
  return c.json(entry);
});

app.get("/offramp/countries", (c) => c.json(OFFRAMP_COUNTRIES));

app.get("/offramp/:domain", async (c) => {
  const domain = c.req.param("domain").trim().toLowerCase();
  return c.json(await getOffRampSummary(c.env.DNS_KV, domain));
});

app.post("/offramp/:domain/config", async (c) => {
  const domain = c.req.param("domain").trim().toLowerCase();
  const body = await c.req.json<{ country: string }>();
  try {
    const config = await configureOffRamp(c.env.DNS_KV, domain, body.country);
...
app.post("/offramp/:domain/withdraw", async (c) => {
  const domain = c.req.param("domain").trim().toLowerCase();
  try {
    const receipt = await withdraw(c.env.DNS_KV, domain);
